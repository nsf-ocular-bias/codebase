...
class FairFaceInput(CIFAR10Input):
  """FairFace input from tfds."""
  cfg = copy.deepcopy(CIFAR10Input.cfg)
  cfg.update(
      dict(
          data_dir=None,
          ...
          tfds_name='fairface',
          splits=dict(
              train=dict(num_images=86_744, tfds_split='train'),
              minival=dict(num_images=10_954, tfds_split='test'),
              eval=dict(num_images=10_954, tfds_split='test'),
          )))

class FairFaceAugmentedInput(CIFAR10Input):
  """FairFace input from tfds."""
  cfg = copy.deepcopy(CIFAR10Input.cfg)
  cfg.update(
      dict(
          ...
          num_classes=2,
          multiclass=False,
          tfds_name='fairface_augmented',
          splits=dict(
              train=dict(num_images=140_000, tfds_split='train'),
              minival=dict(num_images=10_954, tfds_split='test'),
              eval=dict(num_images=10_954, tfds_split='test'),
          )))
...
def get_dataset_class(ds_name):
  return {
      ...
      'cars': CarsInput,
      'fairface': FairFaceInput,
      'fairface-augmented': FairFaceAugmentedInput,
  }[ds_name]
...
@ds_register
class FairFaceFt(ImagenetFt):
  """Finetune fairface configs."""
  cfg = hparams.Config(
      model=dict(
          dropout_rate=0.000001,
          survival_prob=0.8,
